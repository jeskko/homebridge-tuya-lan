class BaseAccessory {
    ...
    _detectColorFunction(value) {
        ...
        if (!this.colorFunction && value) {
            ...
            if (this.colorFunction) console.log(`[TuyaAccessory] Color format for ${this.device.context.name} (${this.device.context.version}) identified as ${this.colorFunction} (length: ${value.length}).`);
        }
        if (!this.colorFunction) {
            this.colorFunction = 'Unknown';
            console.log(`[TuyaAccessory] Color format for ${this.device.context.name} (${this.device.context.version}) is undetectable.`);
    ...
    convertBrightnessFromHomeKitToTuya(value) {
        const min = this.device.context.minBrightness || 10;
        const scale = this.device.context.scaleBrightness || 1000;
